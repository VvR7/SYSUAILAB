...
class MLP:
    def __init__(self, input_dim, hidden_dim1, hidden_dim2, output_dim, lr=0.01):
        ...
        self.w3 = np.random.randn(hidden_dim2, output_dim) * np.sqrt(2 / hidden_dim2)
    ...
    def backward(self, y_true):
        ...
        # 输出层梯度
        grad_z3=2 * (self.z3 - y) / batch_size
    ...
    def train(self,x_train,y_train,x_val,y_val,epochs,lr,batch_size=32,discount_rate=0.9999,constrant=1e-7):
        self.lr=lr
        min_lr=1e-7
...
model=MLP(input_dim=2,hidden_dim1=64,hidden_dim2=32,output_dim=1)
model.train(x_train,y_train,x_val,y_val,epochs=30000,lr=1e-2,batch_size=32)
